...
class MapExample:
    ...
    async def show_map(self, _client: Client):
        self.map = ui.leaflet().classes("w-full h-96")
        selection = ui.select(
            self.locations,
            on_change=lambda e: self.map.set_location(
                e.value, zoom_level=self.zoom_level
            ),
        ).classes("w-40")
        zoom_selection = ui.select(
            self.zoom_levels, on_change=self.on_change_zoom_level
        ).classes("w-40")
        await self.map.initialized()
        selection.set_value(
            next(iter(self.locations))
